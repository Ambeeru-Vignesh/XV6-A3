...
int procstat(int processid, struct pstat *pstat)
{
  struct proc *p;
  int havechild;
  struct proc *curproc = myproc();
...
int ps(void)
{
  ...
  for (prostruct = ptable.proc; prostruct < &ptable.proc[NPROC]; prostruct++)
  {
    ...
    cprintf("%d \t\t %s \t\t %s \t\t %d \t\t\t  %d \t\t\t %d \n",
            prostruct->pid, prostruct->state == SLEEPING ? "SLEEPING" : prostruct->state == RUNNING ? "RUNNING"
                                                                    : prostruct->state == ZOMBIE    ? "ZOMBIE"
                                                                                                    : "WAITING",
